        ...
        <div class="row justify-content-center">
            <div class="col-xl-5 col-lg-6 col-md-8 col-12">
                ...
                <div class="text-center mb-4 animate__animated animate__fadeInDown">
                    ...
                    <p class="text-muted small fw-bold text-uppercase letter-spacing-1">{{ __('Security Clearance') }}</p>
                </div>

                <!-- Modern Card -->
                <div class="modern-card animate__animated animate__zoomIn">
                    <div class="auth-card-header mb-4">
                        <h2 class="h4 fw-bold mb-2">{{ __('Confirm Password') }}</h2>
                        <p class="text-muted small">
                            {{ __('This is a secure area of the application. Please confirm your professional password before continuing.') }}
                        </p>
                    </div>

                    <form method="POST" action="{{ route('password.confirm') }}" class="career-form">
                        @csrf

                        <!-- Password Input -->
                        <div class="mb-5">
                            <label class="career-label">{{ __('Professional Password') }}</label>
                        ...
                        <div class="d-grid mb-4">
                            <button type="submit" class="btn-career-primary py-3">
                                <span>{{ __('Confirm Identity') }}</span>
                                <i class="bi bi-shield-lock ms-2 small"></i>
                            </button>
                        </div>
                        ...
                        @if (Route::has('password.request'))
                            <div class="text-center">
                                <a class="career-link-small" href="{{ route('password.request') }}">
                                    {{ __('Having trouble? Reset your password') }}
                                </a>
                            </div>
                        @endif
                    </form>
